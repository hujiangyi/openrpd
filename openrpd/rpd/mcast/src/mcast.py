...
class Mcast(object):
    ...
    def join(self, session):
        ...
        if session not in self.sessionList:
            ...
            self.logger.info('Mcast Group[%s], port[%d], src[%s], local_ip[%s], session:%s  is  joined'
                             % ('None' if not self.grp_ip else self.grp_ip,
                                0 if not self.port else self.port,
                                "None" if not self.src_ip else self.src_ip,
                                "None" if not self.local_ip else self.local_ip, str(session)))

        if self.status == self.JOINED:
            self.lastchange = time.time()
            self.logger.info('Mcast Group[%s], port[%d], src[%s], local_ip[%s] already joined'
                             % ('None' if not self.grp_ip else self.grp_ip,
                                0 if not self.port else self.port,
                                "None" if not self.src_ip else self.src_ip,
                                "None" if not self.local_ip else self.local_ip))
            return
        else:
            self._join()
    ...
    def leave(self, session):
        ...
        if session in self.sessionList:
            ...
            self.logger.info('Mcast Group[%s], port[%d], src[%s], local_ip[%s], session:%s  is  leaved'
                             % ('None' if not self.grp_ip else self.grp_ip,
                                0 if not self.port else self.port,
                                "None" if not self.src_ip else self.src_ip,
                                "None" if not self.local_ip else self.local_ip, str(session)))
    ...
    def rejoin(self):
        ...
        try:
            ...
            self.logger.warning(
                "exception happen when rejoin grp_ip[%s],src_ip[%s],local[%s]: %s",
                self.grp_ip, self.src_ip, self.local_ip, str(e))
            return False
        self.logger.info('successfully rejoin Mcast instance Group[%s], port[%d], src[%s], local_ip[%s]'
                         % ('None' if not self.grp_ip else self.grp_ip,
                            0 if not self.port else self.port,
                            "None" if not self.src_ip else self.src_ip,
                            "None" if not self.local_ip else self.local_ip))
    ...
    def _join(self):
        ...
        try:
            ...
            self.logger.warning(
                "exception happen when add grp_ip[%s],src_ip[%s],local[%s]: %s",
                self.grp_ip, self.src_ip, self.local_ip, str(e))
            return

        self.status = self.JOINED
        self.logger.info('successfully create Mcast instance Group[%s], port[%d], src[%s], local_ip[%s]'
                         % ('None' if not self.grp_ip else self.grp_ip,
                            0 if not self.port else self.port,
                            "None" if not self.src_ip else self.src_ip,
                            "None" if not self.local_ip else self.local_ip))
    ...
    def _leave(self):
        if self.status == self.JOINED:
            ...
            try:
                ...
                self.logger.warning(
                "exception happen when leave mcast grp_ip[%s],src_ip[%s],local[%s]: %s",
                self.grp_ip, self.src_ip, self.local_ip, str(e))
                return
            self.logger.info('successfully leave group %s' % self.grp_ip)
            self.status = self.LEAVED
        else:
            self.logger.warning(
                'group %s is not in joined state' % self.grp_ip)
