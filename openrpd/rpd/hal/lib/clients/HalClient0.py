...
class HalClient(object):
    ...
    def __init__(self, appName, appDesc, appVer, interestedNotification,logConfigurePath=None,supportedMsgType=[]):
        ...
        if not isinstance(interestedNotification, tuple) and not isinstance(interestedNotification, list):
            raise HalClientError("interestedNotification should be a tuple or list")
    ...
    def _sendMsg(self, msg):
        if self.pushSock:
            ...
            self.logger.error(
                "Cannot send the msg since the push socket is None, msg:%s", str(msg))
    ...
    def sendNotificationMsg(self, notificationType, notificationPayload):
        ...
        notfication = HalMessage(
            "HalNotification", ClientID=self.clientID, HalNotificationType=notificationType,
    ...
    def sendInterestedNotifications(self, notifications):
        ...
        configMsg = HalMessage(
            "HalClientInterestNotificationCfg", ClientID=self.clientID,
    ...
    def sendCfgMsg(self, cfgMsgType, cfgMsgContent):
        ...
        msg = HalMessage(
            "HalConfig", SrcClientID=self.clientID, SeqNum=self.seqNum,
    ...
    def setHalDebugLevel(self, module, level):
        ...
        if self.disconnected:
            ...
            return
        msg = HalMessage("HalSetLoggingLevel", ClientID=self.clientID,
                         Module=module,
                         LoggingLevel=level)
